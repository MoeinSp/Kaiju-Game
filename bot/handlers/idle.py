...
from bot.buttons import BUILD, CONFIRM, back_btn, btn
...
def _render(idle_st: dict, dg_st: dict) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if dg_st["can_run"]:
        rows.append([btn(f"{dg['emoji']} ورود به دخمه‌ی امروز", style=BUILD, callback_data="idle_dungeon")])
    else:
...
def _dungeon_sync(tg_user):
    user, _ = get_or_create_user(tg_user)
    reward = idle.run_dungeon(user)
    return reward, idle.idle_status(user), idle.dungeon_status(user)


async def idle_dungeon_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    reward, idle_st, dg_st = await run_db(_dungeon_sync, update.effective_user)
    if reward is None:
        await query.answer("دخمه‌ی امروزو قبلاً رفتی.", show_alert=True)
        return
    await query.answer("⚔️ فتح شد!")
    text, keyboard = _render(idle_st, dg_st)
    await safe_edit_message_text(
        query,
        f"⚔️ <b>دخمه فتح شد!</b>\n🎁 <b>{_reward_text(reward)}</b>\n\n━━━━━━━━━━\n" + text,
        parse_mode="HTML",
        reply_markup=keyboard,
    )
