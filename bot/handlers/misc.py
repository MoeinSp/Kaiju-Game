...
def _build_help_text() -> str:
    ...
    return (
        ...
        f"({int(constants.ARENA_LOOT_PERCENT * 100)}٪ طلای حریف)\n"
        ...
        "«دوئل» — دوئل خودکار (ریپلای روی پیام حریف؛ «دوئل ۱۰۰» یعنی شرط ۱۰۰ طلا)\n"
        ...
        "«انتقال کایجو <کد>» یا «انتقال هیولا <کد>» — دادن هیولا\n"
        "«انتقال تجهیزات <کد>» — دادن تجهیزات\n"
        f"<blockquote>کدِ هیولا/تجهیزات رو توی پیوی از «کلکسیون»/«تجهیزات» می‌بینی. "
        f"گیرنده الماس می‌ده و تأیید می‌کنه؛ برای هر دو طرف {constants.TRANSFER_COOLDOWN_HOURS} ساعت کول‌داون داره.</blockquote>\n\n"
        f"{get_emoji('energy')} <b>انرژی:</b> شکار، آرنا، دوئل و حمله به رید انرژی مصرف می‌کنن. "
