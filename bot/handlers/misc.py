...
def _build_help_text() -> str:
    return (
        ...
        f"• /campaign — نبردهای دخمه و مراحل ۳ به ۳ ({get_emoji('dungeon', '🗺')})\n"
        f"• /team — چیدن و مدیریت تیم ۳ نفره برای دخمه\n"
        ...
        "• /blacksmith — آهنگری و ارتقای سطح تجهیزات با طلا یا ترکیب هم‌نوع\n"
        f"• /biocrate — صندوق ژنتیکی شانسی ({get_emoji('biocrate')})\n"
        f"• /diamondbox — صندوق‌های الماسی با تضمین دریافت هیولا ({get_emoji('diamond_box')})\n"
        f"• /buildings — ساخت و ارتقای استخراج‌کننده‌های طلا، دی‌ان‌ای و الماس ({get_emoji('building')})\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"{get_emoji('alliance')} <b>اتحاد و جامعه</b>\n"
        "• /alliance_info — مشخصات اتحاد، اعضا، ساختمان‌ها و خزانه\n"
        ...
        "• «حمله» (حمله به باس گروه یا ریپلای روی بازیکن)\n"
...
def _group_help_text() -> str:
    return (
        ...
        "• <b>حمله</b> — حمله به باس فعال گروه (یا با ریپلای، نبرد با بازیکن دیگر)\n"
        ...
        f"<blockquote>💡 تمام کارهای پرورش، ساخت‌وساز، آهنگری و خرید در <b>پیوی ربات</b> انجام می‌شه.</blockquote>"
    )


async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """The single /help handler for every chat type. In a group it acts exactly like
    sending «راهنما» (interactive help card + auto-cleanup); in DM it opens the interactive guide."""
    chat = update.effective_chat
    if chat is not None and chat.type in ("group", "supergroup"):
        from bot.handlers import group_words
        from bot.utils import run_db

        data = await run_db(group_words._card_sync, update.effective_user, chat, "help")
        text, keyboard = group_words._render("help", data)
        sent = await update.message.reply_text(text, parse_mode="HTML", reply_markup=keyboard)
        group_words._schedule_cleanup(context, chat.id, [update.message.message_id, sent.message_id], "help")
        return
