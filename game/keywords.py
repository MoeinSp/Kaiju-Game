...
KEYWORD_DEFS: dict[str, tuple[str, str, str, tuple[str, ...]]] = {
    ...
    "creature": (
        ...
        (
            ...
            "درجه‌ی نایابی (معمولی → اساطیری) هم ضریب استت داره؛ از باکس‌ها و تکثیر می‌آد.",
        ),
    ),
    "equipment": (
        "تجهیزات",
    ...
    "attack": (
        ...
        "حمله به باس گروه",
        (
            "فقط وقتی کار می‌کنه که یه باس فعال توی گروه باشه (با «احضار» بیارش).",
            "هر حمله ۱ ⚡ انرژی می‌خوره.",
            "دمیجی که می‌زنی ثبت می‌شه و سهم غنیمتت رو تعیین می‌کنه.",
        ),
    ),
    "duel": (
        "دوئل",
    ...
    "mine": (
        ...
        (
            ...
            "هیولای فعال و هیولاهای در حال تکثیر نمی‌تونن کار کنن.",
    ...
    "breeding": (
        "تکثیر",
        "egg",
        "تکثیر — هیولای جدید بدون سوزوندن والدین",
        (
            "برعکس ترکیب: والدین <b>سالم می‌مونن</b>، ولی چند ساعت مشغولن.",
            "هرچی والدین نایاب‌تر باشن، زمانش بیشتره (۴ تا ۳۶ ساعت).",
            "هم‌عنصر یا هم‌نژاد بودن و قدرت بالاتر، شانس درجه‌ی بهتر رو زیاد می‌کنه.",
            "والدینِ مشغول نه می‌جنگن نه سر کار می‌رن. انجامش توی پیویه.",
        ),
    ),
    # ── group standing ──────────────────────────────────────────────────────
    "leaderboard": (
...
# Pure teaching pages — no trigger word behind them. The word list explains *what
# to type*; these explain *how the game works*, which is the part a group member
# can't pick up by watching. Kept separate from KEYWORD_DEFS because they aren't
# commands: mixing them in would put un-typeable entries in the word list.
HELP_TOPICS: dict[str, tuple[str, str, str, tuple[tuple[str, str], ...]]] = {
    "creatures": (
        "creature",
